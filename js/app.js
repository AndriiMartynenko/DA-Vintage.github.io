...
if (location.hash) {
	...
	if (document.querySelector('.popup_' + hsh)) {
		...
		_goto(document.querySelector('.' + hsh), 800, '');
	}
}
//=================
//Menu
...
window.onload = function () {
	...
	function documentActions(e) {
		...
		// Прокрутка до нужной секции при клике на меню
		if (target.closest('.menu__body')) {
			if (target.classList.contains('menu__link')) {
				if (target.classList.contains('_goto')) {
					e.preventDefault();
	...
	const animation = (className) => {
		...
		const animatesElements = document.querySelectorAll(`.${className}`);
		animatesElements.forEach(
			item => {
